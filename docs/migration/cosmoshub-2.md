...
[Ochain](https://github.com/onomyprotocol/ochain) application have been split into separate
repositories. This allows for both the Cosmos SDK and Ochain to evolve naturally
and independently. Thus, any future [releases](https://github.com/onomyprotocol/ochain/releases)
...
on its current versioning path (i.e. v0.36.x ) and the [Ochain](https://github.com/onomyprotocol/ochain)
application will become v2.0.x.

__[Ochain](https://github.com/onomyprotocol/ochain) application v2.0.3 is
...
can be done by backing up the `.ochain` directories.

It is critically important to back-up the `.ochain/data/priv_validator_state.json` file after stopping your ochaind process. This file is updated every block as your validator participates in a consensus rounds. It is a critical file needed to prevent double-signing, in case the upgrade fails and the previous chain needs to be restarted.
...
2. Export existing state from `cosmoshub-2`:
   ...
   **NOTE**: It is recommended for validators and operators to take a full data snapshot at the export
   ...
   Before exporting state via the following command, the `ochaind` binary must be stopped!

   ```bash
   $ ochaind export --for-zero-height --height=2902000 > cosmoshub_2_genesis_export.json
   ```
...
v2.0.3 of [Ochain](https://github.com/onomyprotocol/ochain).

   **NOTE**: Go [1.13+](https://golang.org/dl/) is required!

   ```bash
   $ git clone https://github.com/onomyprotocol/ochain.git && cd ochain && git checkout v2.0.3; make install
   ```
...
6. Migrate exported state from the current v0.34.6+ version to the new v2.0.3 version:
   ...
   ```bash
   $ ochaind migrate v0.36 cosmoshub_2_genesis_export.json --chain-id=cosmoshub-3 --genesis-time=[PLACEHOLDER]> genesis.json
   ```
...
10. Move the new `genesis.json` to your `.ochain/config/` directory
11. Replace the `db_backend` on `.ochain/config/config.toml` to:

    ```toml
    db_backend = "goleveldb"
    ```

12. Note, if you have any application configuration in `ochaind.toml`, that file has now been renamed to `app.toml`:

    ```bash
    $ mv .ochain/config/ochaind.toml .ochain/config/app.toml
    ```
...
4. We highly recommend standing up a [testnet](https://github.com/onomyprotocol/ochain/blob/master/docs/deploy-testnet.md)
